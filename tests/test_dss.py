...
class IdPhenotypeMapTests(unittest.TestCase):
    ...
    def test_retrieving(self):
        ...
        i0.add("HP:1")
        i0.add("HP:4")
        self.assertEqual(i0, self.idp.getPhenotypes("i0"), "retrieving phenotypes")
        hp4 = set()
        hp4.add("i0")
        hp4.add("i1")
        self.assertEqual(hp4, self.idp.getIds("HP:4"), "retrieving ids")
...
class OOMapDataTests(unittest.TestCase):
    ...
    def test_str(self):
        ...
        self.assertEqual(oodstr, str(ood), "simple str representation")
    ...
    def test_update(self):
        ...
        self.assertEqual(oodstr, str(ood), "update with lower score should be ignored")
        ood.checkUpdate(ood3)
        self.assertEqual(
            str(ood3), str(ood), "update with higher score should replace all fields"
        )
...
class OntologyOntologyMapTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        a1 = {"query": "hp0", "match": "mp1", "ic": 1, "simJ": 1, "lcs": "xyz"}
        a2 = {"query": "hp0", "match": "mp2", "ic": 4, "simJ": 4, "lcs": "xyz"}
        a3 = {"query": "hp1", "match": "mp1", "ic": 1, "simJ": 4, "lcs": "xyz"}
        self.oomap = dss.OntologyOntologyMap()
        self.oomap.add(a1)
        self.oomap.add(a2)
        self.oomap.add(a3)

        aaa1 = {"query": "hp0", "match": "hp0", "ic": 1, "simJ": 4, "lcs": "xyz"}
        aaa2 = {"query": "hp0", "match": "hp1", "ic": 1, "simJ": 2, "lcs": "xyz"}
        aaa3 = {"query": "hp1", "match": "hp1", "ic": 1, "simJ": 9, "lcs": "xyz"}
    ...
    def test_ideal_match1(self):
        ...
        self.assertEqual(
            ["hp1"], phenos, "ideal connects with high score hp terms (hp1)"
        )

    def test_ideal_match2(self):
        """Test identifying best inverse matches."""
        phenos = self.oomap.ideal_match_phenotypes(["mp2"])
        self.assertEqual(["hp0"], phenos, "ideal connects with only hp term (hp1)")

    def test_ideal_match3(self):
        """Test identifying best inverse matches."""
        phenos = self.oomap.ideal_match_phenotypes(["mp99"])
        self.assertEqual([], phenos, "ideal is empty because model does not exist")
    ...
    def test_ideal_match4(self):
        ...
        self.assertEqual(["hp0"], ideal0, "ideal should return itself for hp0")
        self.assertEqual(["hp1"], ideal1, "ideal should return itself for hp1")
        self.assertEqual(
            ["hp0", "hp1"], sorted(ideal01), "ideal should return itself hp0, hp1"
        )
...
class GeneInfoTests(unittest.TestCase):
    ...
    def test_official1(self):
        ...
        self.assertEqual(gg.official_symbol, None, "symbol should be empty at first")
        gg.addSymbol("NAME0")
        self.assertEqual(gg.official_symbol, "NAME0", "adding first symbol")
        self.assertEqual(gg.symbols, set(), "first symbol is official symbol")
        gg.addSymbol("NAME1")
        self.assertEqual(
            gg.official_symbol, "NAME1", "adding new symbol replaces previous official"
        )
        self.assertEqual(
            gg.symbols, set(["NAME0"]), "the old official symbol should move to alt set"
        )
    ...
    def test_official2(self):
        ...
        self.assertEqual(
            gg.symbols, set(["Withdrawn"]), "withdrawn symbol should appear in .symbols"
        )
...
class MapSetsTests(unittest.TestCase):
    ...
    def test_has(self):
        ...
        self.assertEqual(
            self.ms.has("a"), True, "instance has key that has been inserted"
        )
        self.assertEqual(self.ms.has("x"), False, "instance does not have other keys ")
        self.assertEqual(self.ms.has("A"), False, "has is case sensitive")

    def test_get(self):
        """Check extraction of sets."""

        self.assertEqual(
            self.ms.get("a"), set(["x", "y"]), "a has two distinct elements"
        )
    ...
    def test_reverse(self):
        ...
        self.assertEqual(
            expected.get("x"), output.get("x"), "x should link to one element"
        )
        self.assertEqual(
            expected.get("y"), output.get("y"), "y should link to two elements"
        )
