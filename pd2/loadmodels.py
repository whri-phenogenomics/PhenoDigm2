...
def readIMPCModels(
    infile, models, addphenotypes=False, pthreshold=1e-4, life_stages=None
):
    ...
    with gzip.open(infile, "rt") as f:
        ...
        for fields in impc_reader:
            ...
            if modelid not in models:
                description = (
                    fields[alleleindex] + " " + zygosity + " " + life_stage_group
                )
                nowmodel = MouseModel(
                    modelid, fields[sourceindex], fields[strainindex], description
                )
...
def loadMGIMouseModelData(db_file, anno_dir, headers_dir, db_dir):
    ...
    maxindex = max(
        [geneindex, phenoindex, bgindex, pubmedindex, alleleindex, modelindex]
    )
