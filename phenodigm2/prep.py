...
def fetchUsingXmlQuery(
    urlpath, querybase, filename=None, querydir=None, outdir=None, skip_if_exists=True
):
...
def runDownloads(config):
    ...
    This function looks for files declared in dependencies.json."""
    ...
    for nowid in dependencies:
        ...
        if "query" in nowset:
            ...
            for q, f in zip(nowset["query"], nowset["filename"]):
                fetchUsingXmlQuery(
                    nowurl, q, filename=f, querydir=nowresdir, outdir=nowdir
                )
        else:
