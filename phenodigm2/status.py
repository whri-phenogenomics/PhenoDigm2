...
def checkDiseases(config):
    ...
    with gzip.open(phenfile, "wt") as f:
        ...
        for did in diseases.keys():
            if diseases[did] == 0:
                f.write(did + "\n")
    warninfo(str(num_nopheno) + " diseases without phenotypes", num_nopheno > 0)
    ...
    warninfo(str(num_missing) + " diseases without descriptions", num_missing > 0)
    ...
    with gzip.open(phenfile, "wt") as f:
        ...
        for mid in models.keys():
            if not phenotypes.has(mid):
                ...
                num_nopheno += 1
    warninfo(str(num_nopheno) + " models without phenotypes", num_nopheno > 0)
    ...
    with gzip.open(genofile, "wt") as f:
        ...
        for mid in models.keys():
            if not genotypes.has(mid):
                ...
                num_nogeno += 1
    warninfo(str(num_nogeno) + " models without genotypes", num_nogeno > 0)
    ...
    with gzip.open(descfile, "wt") as f:
        ...
        for mid in phenotypes.keys():
            if mid not in models:
                ...
                num_nodesc += 1
    warninfo(str(num_nodesc) + " models without descriptions", num_nodesc > 0)
